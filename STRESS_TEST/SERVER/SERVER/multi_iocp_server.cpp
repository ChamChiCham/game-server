...
bool can_see(int a, int b)
{
	...
	return (clients[a].y - clients[b].y) > VIEW_RANGE);*/
}
...
void process_packet(int c_id, char* packet)
{
	...
	case CS_MOVE: {
		...
		clients[c_id]._view_list_lock.lock();
		std::unordered_set<int> old_viewlist = clients[c_id]._view_list;
		clients[c_id]._view_list_lock.unlock();

		std::unordered_set<int> new_viewlist;

		for (auto& cl : clients) {
			if (cl._state != ST_INGAME) continue;
			if (false == can_see(cl._id, c_id)) continue;
